...
class client:
    def __init__(self, x, y, host_ip, port, ArduinoCom):
        self.x = y
        self.y = x
        ...
        if self.connecting:
            try:
                while True:
                    # print(self.pos)
                    if self.if_video:
                        img, self.frame1 = self.vid1.getframe()
                        img2, self.frame2 = self.vid2.getframe()
                    else:
    ...
    def humanDetect(self):
        while self.connecting:
            if self.if_video:
                img, frame1 = self.vid1.getframe()
            else:
                frame1 = cv2.imread("./img/road.jfif")
            frame1 = imutils.resize(frame1, width=380)
            self.bbox, classes, confs, findPerson, frame1 = self.yolo.getDefaultOut(frame1)
